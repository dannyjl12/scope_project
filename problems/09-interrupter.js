...
function interrupter(interruptingWord) {
 ...
 return interuption = function(sentence) {
  let interuptedSentence = '';
  let words = sentence.split(' ');
  interuptedSentence += words[0];

  for (let i = 1; i < words.length; i++) {
    interuptedSentence += ' ' + interruptingWord + ' ' + words[i];
  }
  return interuptedSentence;
 }
}
